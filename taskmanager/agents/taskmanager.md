...
| `taskmanager:expand` | Expand a task or all tasks above a complexity threshold into subtasks |
...
| `taskmanager:update-task` | Update task fields with optional AI-assisted rewriting |
| `taskmanager:update-status` | Batch update task status for one or more tasks efficiently |
| `taskmanager:scope` | Adjust task scope up or down with optional cascade to dependent tasks |
| `taskmanager:tags` | Manage tags: list, add, remove, filter, rename across tasks |
| `taskmanager:dependencies` | Validate and auto-fix task dependencies |
| `taskmanager:move` | Move a task to a different position or parent |
...
.taskmanager/
  ...
  config.json                   # Project configuration (defaults, dashboard, execution settings)
