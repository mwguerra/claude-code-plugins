---
description: Manage pre-run lifecycle hooks (the "beforeAll" autopilot reads before the first test)
...
# /e2e-test-specialist:before-all

Manage entries in `lifecycle_hooks WHERE phase='pre-run'`. These are
project-specific instructions the autopilot reads and follows **before the
first test of every run** (clean-slate verification, environment warm-up,
credential rotation, baseline snapshots, etc.).
...
| `--list`                                                    | List active pre-run hooks (ordered by `order_idx`)          |
...
# === MODE: --list =========================================================
if [[ -n "${LIST:-}" ]]; then
    e2e_section "Pre-run hooks (active=1, ordered by order_idx)"
...
if [[ -n "${SHOW_ID:-}" ]]; then
    ...
    sqlite3 -bail -line "$E2E_DB" "
      SELECT id, phase, title, enforcement, order_idx, active,
        ...
        FROM lifecycle_hooks
       WHERE id = $(e2e_sql_quote "$SHOW_ID") AND phase='pre-run';
    "
    exit 0
fi
...
# === MODE: --remove <id> ==================================================
if [[ -n "${REMOVE_ID:-}" ]]; then
    EXISTS="$(e2e_query_value "SELECT COUNT(*) FROM lifecycle_hooks WHERE id=$(e2e_sql_quote "$REMOVE_ID") AND phase='pre-run';")"
    [[ "$EXISTS" -eq 1 ]] || e2e_die "no pre-run hook with id '$REMOVE_ID'"
    e2e_exec "
      UPDATE lifecycle_hooks
         SET active=0, updated_at=datetime('now')
       WHERE id=$(e2e_sql_quote "$REMOVE_ID") AND phase='pre-run';
    "
    echo "Disabled pre-run hook: $REMOVE_ID"
    exit 0
fi

# === MODE: upsert =========================================================
[[ -n "${TITLE:-}" ]] || e2e_die "usage: /e2e-test-specialist:before-all <title> --body \"…\"  (or --from <file>)  |  --list  |  --show <id>  |  --remove <id>"
...
echo "Saved pre-run hook: $HOOK_ID"
echo "  title       : $TITLE"
echo "  enforcement : $ENFORCEMENT"
echo "  order_idx   : $ORDER_IDX"
echo "  body bytes  : ${#BODY}"
echo
echo "View with:    /e2e-test-specialist:before-all --show $HOOK_ID"
echo "Disable with: /e2e-test-specialist:before-all --remove $HOOK_ID"
echo "List all:     /e2e-test-specialist:before-all --list"
```

## Examples

```bash
# Verify the VPS is in clean slate before any test runs (blocking).
/e2e-test-specialist:before-all "Verify VPS clean slate" \
  --body "SSH to wopr.test and run \`panel-status --json\`. If any sites or droplets exist from a prior run, abort: the operator must run Phase 0 manually." \
  --enforcement blocking --order 10

# Long body from a file (markdown is fine).
/e2e-test-specialist:before-all "Pre-run smoke check" --from .e2e-testing/hooks/pre-smoke.md --order 20

# Update an existing hook in place by re-using its id.
/e2e-test-specialist:before-all "Verify VPS clean slate (v2)" \
  --id lh-pre-verify-vps-clean-slate \
  --body "Updated procedure…" --enforcement blocking --order 10

# List, inspect, disable.
/e2e-test-specialist:before-all --list
/e2e-test-specialist:before-all --show lh-pre-verify-vps-clean-slate
/e2e-test-specialist:before-all --remove lh-pre-verify-vps-clean-slate
```

## When the autopilot reads these

`/e2e-test-specialist:autopilot` step 3.5 — after run setup, before the
first `/test` invocation. Hooks run in `order_idx` ASC, then `id` ASC.
A `blocking` hook that fails halts the autopilot (active run stays
`in-progress`); an `advisory` hook that fails logs + writes a memory and
the autopilot continues.

## See also

- `/e2e-test-specialist:after-all` — the post-run counterpart
